...
void pcd_to_teaser(pcl::PointCloud<pcl::PointXYZ>::Ptr& input_cloud,
                   teaser::PointCloud& output_cloud) {
...
int main() {
  ...
  std::string sourcePath = "H:/workspace/ImageLocation/root/templates/areaCamera/SS4B(A)_GLOC_1/SS4B(A)_GLOC_1.pcd";
  std::string targetPath = "H:/workspace/ImageLocation/data/SS4B(A)_BOTTOM_B3_GLOC2_921_1/SS4B(A)_BOTTOM_B3_GLOC2_921_1.pcd";
  ...
  if (pcl::io::loadPCDFile<pcl::PointXYZ>(sourcePath, *sourceCloud) == -1) {
    PCL_ERROR("load error \n");
    return (-1);
  }

  if (pcl::io::loadPCDFile<pcl::PointXYZ>(targetPath, *targetCloud) == -1) {
    PCL_ERROR("load error \n");
    return (-1);
  }
  pcd_to_teaser(sourceCloud, src_cloud);
  pcd_to_teaser(targetCloud, target_cloud);
